...
export class ClientComponent implements OnInit {
    ngAfterViewInit(): void {
      // Inicializar el carrusel Bootstrap manualmente
      const el = document.getElementById('carouselNavidad');
      if (el && (window as any).bootstrap) {
        try {
          ...
          console.warn('No se pudo inicializar el carrusel:', e);
        }
      }
    }
  products: Product[] = [];
  ...
  goToProductDetail(productId: number): void {
    // Navegar y scrollear al tope cuando la navegación se complete
