...
export class LoginComponent {
    // Log para apertura/cierre de modal admin
    openAdminModal() {
      this.showAdminModal = true;
      console.log('[UI] Modal admin abierto');
    }
    closeAdminModal() {
      this.showAdminModal = false;
      console.log('[UI] Modal admin cerrado');
    }

    // Log para cambios en campos de email/contraseña
    onClientEmailChange(val: string) {
      this.clientEmail = val;
      console.log('[UI] Cambio en clientEmail:', val);
    }
    onClientPasswordChange(val: string) {
      this.clientPassword = val;
      console.log('[UI] Cambio en clientPassword:', val ? '(proporcionada)' : '(vacía)');
    }
    onAdminEmailChange(val: string) {
      this.adminEmail = val;
      console.log('[UI] Cambio en adminEmail:', val);
    }
    onAdminPasswordChange(val: string) {
      this.adminPassword = val;
      console.log('[UI] Cambio en adminPassword:', val ? '(proporcionada)' : '(vacía)');
    }
  clientEmail: string = '';
  ...
  fillDemoCredentials(): void {
    ...
    console.log('[UI] Se llenaron credenciales demo de cliente');
  }

  fillDemoAdmin(): void {
    this.adminEmail = 'admin@mass.com';
    this.adminPassword = 'password123';
    console.log('[UI] Se llenaron credenciales demo de admin');
  }
}
